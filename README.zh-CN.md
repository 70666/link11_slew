...
> **Codex 能构建的不只是软件.** 如果你仍然认为 AI 辅助工程只适用于软件开发, 这个仓库展示了另一条路径: 由工程师主导, AI 协助完成 FPGA 架构设计, 延时敏感 RTL 实现和真实硬件验证.

这是一个由 ChatGPT 5.5 和 Codex 协同设计的可综合 Link 11 SLEW FPGA 发送与接收链路开源实现. AI 参与了系统架构探索, 维护者不熟悉的解调方案设计, 复杂 RTL 实现, 仿真分析和反复调试. 维护者审核每项设计决策, 并通过仿真和 ZU47DR 上板测试验证最终调制解调器.

项目覆盖帧构建, CRC, 卷积编码, 交织, 加扰, IQ 波形生成, 符号同步, 前导码相关, 频偏校正, QPSK 解调, Viterbi 译码和 CRC 校验.
