...
`include "preamble_iq_wire.vh"
`PREAMBLE_IQ_WIRE_DECLARE
...
generate
    for(s = 0; s < PREAMBLE_SYMBOL_NUM - KNOWN_SEQUENCE_END_SYMBOL; s = s + 1) begin
        assign preamble_i[s] = PREAMBLE_I[s+KNOWN_SEQUENCE_END_SYMBOL+1];
        assign preamble_q[s] = PREAMBLE_Q[s+KNOWN_SEQUENCE_END_SYMBOL+1];
    end
endgenerate
